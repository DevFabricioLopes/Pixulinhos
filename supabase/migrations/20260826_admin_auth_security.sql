...
-- Protect CMS tables without changing their data.
DO $$
DECLARE t text;
BEGIN
  FOREACH t IN ARRAY ARRAY[
    ...
    IF to_regclass(format('public.%I', t)) IS NOT NULL THEN
      ...
      EXECUTE format(
        'CREATE POLICY "Pixulinhos public read %s" ON public.%I FOR SELECT TO anon, authenticated USING (true)',
        t, t
      );

      EXECUTE format(
        'CREATE POLICY "Pixulinhos admin write %s" ON public.%I FOR ALL TO authenticated USING (public.is_pixulinhos_admin()) WITH CHECK (public.is_pixulinhos_admin())',
        t, t
      );
    END IF;
  END LOOP;
...
IF EXISTS (SELECT 1 FROM pg_class WHERE oid = 'public.orders'::regclass) THEN
  ALTER TABLE public.orders ENABLE ROW LEVEL SECURITY;
  DROP POLICY IF EXISTS "Pixulinhos admin orders" ON public.orders;
  CREATE POLICY "Pixulinhos admin orders"
    ON public.orders
    FOR ALL
    TO authenticated
    USING (public.is_pixulinhos_admin())
    WITH CHECK (public.is_pixulinhos_admin());
END IF;
...
CREATE POLICY "Pixulinhos admin storage insert"
  ON storage.objects FOR INSERT TO authenticated
  WITH CHECK (
    bucket_id IN ('pixulinhos-media','pixulinhos-products','pixulinhos-banners')
    AND public.is_pixulinhos_admin()
  );

CREATE POLICY "Pixulinhos admin storage update"
...
CREATE POLICY "Pixulinhos admin storage delete"
